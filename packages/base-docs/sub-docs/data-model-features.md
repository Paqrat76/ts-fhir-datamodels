...
JSON representations of resource data will not have keys/values for any missing ("optional" and "requires") data elements.
...
Data elements that do not have a value (are `undefined` or `null`) are omitted from the JSON object.

The `static parse()` method will throw a `FhirError` if the provided JSON representation is not a valid JSON
object, the JSON representation is malformed, or the data elements cannot be parsed into the expected data types.
...
- `AssertionError` - Error thrown when an assertion fails.
  ...
  They are also used when validating input data via accessors.
...
The `FhirError` is thrown by the `static parse()` method if the provided JSON representation is not a valid JSON
object, the JSON representation is malformed, or the data elements cannot be parsed into the expected data types.
